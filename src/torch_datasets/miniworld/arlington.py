...
class Arlington(Base, ABC):
    ...
    # pixels_per_class = [3867608, 1172392]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.labeled_image_paths = sorted(
            glob.glob(f'{self.data_path}/Arlington/train/*_x.png')
        ) + sorted(
            glob.glob(f'{self.data_path}/Arlington/test/*_x.png')
        )

        self.unlabeled_image_paths = []

        self.label_paths = sorted(
            glob.glob(f'{self.data_path}/Arlington/train/*_y.png')
        ) + sorted(
            glob.glob(f'{self.data_path}/Arlington/test/*_y.png')
        )
