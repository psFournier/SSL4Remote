...
class Toulouse(Base, ABC):

    nb_unlabeled_images = 0
    image_size = (3504, 3452)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.labeled_image_paths = sorted(
            glob.glob(f'{self.data_path}/toulouse/train/*_x.tif')
        ) + sorted(
            glob.glob(f'{self.data_path}/toulouse/test/*_x.tif')
        )

        self.unlabeled_image_paths = []

        self.label_paths = sorted(
            glob.glob(f'{self.data_path}/toulouse/train/*_y.tif')
        ) + sorted(
            glob.glob(f'{self.data_path}/toulouse/test/*_y.tif')
        )


class ToulouseLabeled(Toulouse, BaseLabeled):
...
class ToulouseUnlabeled(Toulouse, BaseUnlabeled):
